...
import frc.robot.commands.TempSubwooferCmd;
...
public class RobotContainer {
        ...
        private void configureBindings() {
                ...
                // climberCompactKeybind = new Keybind(codriverController.getHID(),
                // Button.Back);
                                        ...
                                        if (DriverStation.isTeleopEnabled()) {
                                                ...
                                                if (backLimelight.limelightRotation && backLimelight.tagTv) {
                                                        ...
                                                        if (ExtraMath.within(driverController.getRightX(), 0, 0.1)
                                                                ...
                                                                stayDegree = Rotation2d.fromDegrees(pGryo.Y)
                                                                                .rotateBy(allianceBasedRotation());

                                                        } else if (!ExtraMath.within(driverController.getRightX(), 0,
                                                                        0.1)) {
                                                                shouldStayDegree = false;
                                                        }

                                                        rate = ExtraMath.deadzone(
                ...
                snapTo0Keybind.trigger().and(snapTo90Keybind.trigger().negate()).and(snapTo180Keybind.trigger().negate()).and(snapTo270Keybind.trigger().negate()).whileTrue(drivetrain.applyRequest(() -> {
                ...
                //60 degrees
                snapTo0Keybind.trigger().and(snapTo90Keybind.trigger()).and(snapTo180Keybind.trigger().negate()).and(snapTo270Keybind.trigger().negate()).whileTrue(drivetrain.applyRequest(() -> {
                        stayDegree = rotation_60degree;
                        return look
                                        .withTargetDirection(rotation_60degree)
                                        .withVelocityX(driverGetX())
                                        .withVelocityY(driverGetY());
                }));

                //90
                snapTo0Keybind.trigger().negate().and(snapTo90Keybind.trigger()).and(snapTo180Keybind.trigger().negate()).and(snapTo270Keybind.trigger().negate()).whileTrue(drivetrain.applyRequest(() -> {
                ...
                //120
                snapTo0Keybind.trigger().negate().and(snapTo90Keybind.trigger()).and(snapTo180Keybind.trigger()).and(snapTo270Keybind.trigger().negate()).whileTrue(drivetrain.applyRequest(() -> {
                        stayDegree = rotation_120degree;
                        return look
                                        .withTargetDirection(rotation_120degree)
                                        .withVelocityX(driverGetX())
                                        .withVelocityY(driverGetY());
                }));

                //180
                snapTo0Keybind.trigger().negate().and(snapTo90Keybind.trigger().negate()).and(snapTo180Keybind.trigger()).and(snapTo270Keybind.trigger().negate()).whileTrue(drivetrain.applyRequest(() -> {
                ...
                //240
                snapTo0Keybind.trigger().negate().and(snapTo90Keybind.trigger().negate()).and(snapTo180Keybind.trigger()).and(snapTo270Keybind.trigger()).whileTrue(drivetrain.applyRequest(() -> {
                        stayDegree = rotation_240degree;
                        return look
                                        .withTargetDirection(rotation_240degree)
                                        .withVelocityX(driverGetX())
                                        .withVelocityY(driverGetY());
                }));
                
                //270
                snapTo0Keybind.trigger().negate().and(snapTo90Keybind.trigger().negate()).and(snapTo180Keybind.trigger().negate()).and(snapTo270Keybind.trigger()).whileTrue(drivetrain.applyRequest(() -> {
                        stayDegree = rotation_270degree;
                        return look
                                        .withTargetDirection(rotation_270degree)
                                        .withVelocityX(driverGetX())
                                        .withVelocityY(driverGetY());
                }));
                
                //300
                snapTo0Keybind.trigger().and(snapTo90Keybind.trigger().negate()).and(snapTo180Keybind.trigger().negate()).and(snapTo270Keybind.trigger()).whileTrue(drivetrain.applyRequest(() -> {
                        stayDegree = rotation_300degree;
                        return look
                                        .withTargetDirection(rotation_300degree)
                                        .withVelocityX(driverGetX())
                                        .withVelocityY(driverGetY());
                }));
                ...
                flatPassKeybind.trigger()
                                .whileTrue(new SetArmPositionCmd(arm, ArmPosition.Flat));
                flatPassKeybind.trigger()
                                .whileTrue(new SpinUpShooterCmd(shooter, Constants.FLAT_PASS_SHOOT_SPEED, false));
                ...
                intakeKeybind.trigger().and(modifyArm.negate()).and(fixedArm.negate())
                ...
                intakeKeybind.trigger().and(modifyArm.negate()).and(fixedArm)
                                .whileTrue(new FadeawayCmd(floorIntake, shooter, arm, backLimelight, logger));
                ...
                // shootPositionKeybind.trigger().and(modifyArm.negate().and(fixedArm.negate()))
                                // .onFalse(new TempSubwooferCmd(arm, shooter));
                ...
                spitTrigger.trigger().and(modifyArm).and(fixedArm).whileTrue(new ShootCmd(arm, shooter, 1));
        ...
        final Rotation2d rotation_60degree = Rotation2d.fromDegrees(60);
        final Rotation2d rotation_90degree = Rotation2d.fromDegrees(90);
        final Rotation2d rotation_120degree = Rotation2d.fromDegrees(120);
        final Rotation2d rotation_180degree = Rotation2d.fromDegrees(180);
        final Rotation2d rotation_240degree = Rotation2d.fromDegrees(240);
        final Rotation2d rotation_270degree = Rotation2d.fromDegrees(270);
        final Rotation2d rotation_300degree = Rotation2d.fromDegrees(300);
